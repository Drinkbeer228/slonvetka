...
code = code.replace(
  /<div\s*className="space-y-6 mt-3 relative pb-52 sm:pb-56"\s*style={{ paddingBottom: 'calc\\(14rem \+ env\\(safe-area-inset-bottom, 0px\\)\\)' }}\s*onTouchStart={handleTouchStart}\s*onTouchMove={handleTouchMove}\s*onTouchEnd={handleTouchEnd}\s*>/,
...
// Second section (Feed - roughage + rations)
// Let's replace the first feed one and the others manually
