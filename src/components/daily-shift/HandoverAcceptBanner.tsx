...
export function HandoverAcceptBanner({ pendingShift, currentUserId, onAccept, onReject }: HandoverAcceptBannerProps) {
  ...
  const handleAccept = async () => {
    ...
    try {
      await shiftService.acceptHandover(pendingShift, currentUserId);
      onAccept();
    } catch (err) {
    ...
    <div className="fixed top-4 left-4 right-4 z-[90] sm:left-auto sm:right-6 sm:top-6 sm:w-96 bg-white/95 backdrop-blur-xl border border-white rounded-[24px] shadow-[0_8px_30px_rgba(0,0,0,0.12)] overflow-hidden flex flex-col">
      <div className="p-4 flex gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-500 font-medium leading-relaxed">
            Примите дежурство, чтобы начать работу с текущими задачами.
          </p>
      ...
      <div className="flex border-t border-slate-100 divide-x divide-slate-100 bg-slate-50/50">
        <button
          ...
          className="flex-1 py-3.5 flex items-center justify-center gap-2 text-slate-500 font-bold text-xs uppercase tracking-wider hover:bg-slate-100 hover:text-slate-700 transition-colors disabled:opacity-50"
        >
        ...
        <button
          ...
          disabled={isProcessing}
          className="flex-1 py-3.5 flex items-center justify-center gap-2 text-emerald-600 font-bold text-xs uppercase tracking-wider hover:bg-emerald-50 transition-colors disabled:opacity-50"
        >
          {isProcessing ? (
