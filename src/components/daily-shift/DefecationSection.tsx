...
interface DefecationSectionProps {
  elephants?: Elephant[];
...
export function DefecationSection({
  elephants,
  metrics,
  onMetricChange,
  isLocked = false
}: DefecationSectionProps) {
    ...
    if (metrics) {
      setLocalCounts(prev => {
        ...
        Object.entries(metrics).forEach(([id, m]) => {
          if (m?.poop_count !== undefined) {
            next[id] = m.poop_count;
            if (m.elephant_id) {
              next[m.elephant_id] = m.poop_count;
            }
          }
        });
        return next;
  ...
  const getCount = (elephantId: string): number => {
    ...
    if (metrics?.[elephantId]?.poop_count !== undefined) {
      return metrics[elephantId].poop_count;
    }
    const found = Object.values(metrics || {}).find(m => m?.elephant_id === elephantId);
    if (found?.poop_count !== undefined) {
      return found.poop_count;
    }
    return 0;
  };

  // Total feces count across all 3 elephants
  const totalFeces = displayElephants.reduce((sum, el) => sum + getCount(el.id), 0);

  // Sync traits with metrics
  useEffect(() => {
    const firstElephantId = displayElephants[0]?.id;
    if (firstElephantId && metrics?.[firstElephantId]?.feces_traits) {
      setLocalTraits(metrics[firstElephantId].feces_traits);
    }
  ...
  const activeTraits = localTraits.length > 0
    ? localTraits
    : (metrics && displayElephants[0] && metrics[displayElephants[0].id]?.feces_traits) || ['Сформирован (норма)'];
  ...
  const handleIncrement = (elephantId: string) => {
    ...
    onMetricChange?.(elephantId, 'poop_count', nextVal);
  };
  ...
  const toggleTrait = (trait: string) => {
    ...
    displayElephants.forEach(e => {
      onMetricChange?.(e.id, 'feces_traits', newTraits);
    });
  };
  ...
  // Find stool photos across all elephants
  const stoolPhotos = displayElephants.flatMap(e => {
    const photos = metrics?.[e.id]?.photos || [];
    return photos.filter(p => p.section === 'stool').map(p => ({ elephant: e, photo: p }));
  });

  const latestPhoto = stoolPhotos[stoolPhotos.length - 1];
  ...
  return (
    <div className="bg-white/80 backdrop-blur-2xl border border-white/80 rounded-[28px] p-3.5 sm:p-5 shadow-[0_8px_32px_rgba(0,0,0,0.04)] space-y-4">
      ...
      {/* Header with Title, Total Count Badge, and PhotoActionPattern in top-right corner */}
      <div className="flex items-center justify-between gap-2 min-w-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="flex items-center gap-1.5 flex-wrap min-w-0">
            ...
            <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100/90 border border-slate-200/70 text-slate-700 shrink-0">
              ...
              <span className="text-xs font-black text-slate-900">{totalFeces}</span>
            </div>
          </div>
        </div>
        ...
        <div className="shrink-0">
          <PhotoActionThumbnail
            ...
            isLocked={isLocked}
            title={latestPhoto ? `Фото дефекации (${latestPhoto.elephant.name})` : 'Зафиксировать на фото'}
      ...
      {/* 2. Three Vertical Counters Grid */}
          ...
          return (
            <div key={elephant.id} className="flex flex-col">
              ...
              <button
                ...
                className="h-12 w-full rounded-t-2xl bg-white/80 hover:bg-white active:scale-95 active:bg-amber-100/60 border border-b-0 border-slate-200/80 flex items-center justify-center text-2xl font-bold text-slate-800 disabled:opacity-40 disabled:cursor-not-allowed transition-all touch-manipulation cursor-pointer select-none shadow-sm"
                aria-label={`Увеличить кучи для ${elephant.name}`}
              >
                +
              </button>
              ...
              <button
                ...
                aria-label={`Уменьшить кучи для ${elephant.name}`}
              >
                -
              </button>
            </div>
      ...
      {/* 3. Consistency / Stool Traits Chips */}
      <div className="pt-1">
        <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">
          Консистенция и особенности
        </div>
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-200">
          <div className="w-full max-w-sm bg-white/95 backdrop-blur-2xl border border-white/80 rounded-[32px] p-6 shadow-2xl animate-in zoom-in-95 duration-200 space-y-6">
            <div className="text-center space-y-1">
              <h3 className="text-xl font-black text-slate-800">Чей стул на фото?</h3>
            ...
            <div className="flex flex-col gap-2.5">
              {displayElephants.map(e => (
                <button
                  ...
                  className="w-full min-h-[50px] py-3 bg-white/80 border border-slate-200/80 hover:border-emerald-300 hover:bg-emerald-50 text-slate-800 hover:text-emerald-800 rounded-2xl font-bold text-base transition-all active:scale-[0.98] shadow-xs flex items-center justify-center cursor-pointer"
                >
                  {e.name}
                </button>
              ))}
      ...
      {/* Lightbox / Preview Modal for Stool Photo */}
        ...
        <div className="fixed inset-0 z-[160] flex flex-col bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-200">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="font-bold text-sm">
                  Фото дефекации ({previewModalPhoto.elephant.name})
                </div>
          ...
          <div className="flex-1 w-full p-4 flex items-center justify-center overflow-hidden">
            <img
              ...
              alt={`Дефекация ${previewModalPhoto.elephant.name}`}
