...
export function DailyShiftPage({ onNavigate }: { onNavigate: (screen: string) => void }) {
  const { profile, elephants, selectedDate, setSelectedDate, fodderInventory, updateFodderAmount, deductFodderKg } = useStore();
  const [shift, setShift] = useState<DailyShift | null>(null);
  
  // App Shell States (7 screens: Physio, Roughage, Kitchen, Fodder, Chores, Calendar, Menu)
  ...
  const slideWrapperClass = "w-screen min-w-full max-w-full h-[100dvh] flex-shrink-0 snap-center snap-always flex flex-col overflow-y-auto overscroll-y-contain px-4 pt-[calc(env(safe-area-inset-top)+2.5rem)] pb-20 text-slate-100";
  ...
  return (
    <div ref={scrollContainerRef} className="fixed inset-0 w-full h-[100dvh] flex flex-row overflow-x-auto overflow-y-hidden snap-x snap-mandatory scroll-smooth bg-slate-950 touch-pan-x select-none" onScroll={handleScroll}>
      ...
      {/* ULTRA-MINIMALISTIC GLOBAL HEADER (ONLY AVATAR, 7 REELS INDICATORS & SYSTEM TIME) */}
      <div className="fixed top-0 left-0 right-0 z-40 bg-gradient-to-b from-slate-950 via-slate-950/95 to-transparent px-3 pt-[calc(env(safe-area-inset-top)+0.4rem)] pb-2 pointer-events-auto">
        <div className="flex items-center gap-2.5 max-w-lg mx-auto w-full">
          {/* 1. Compact Avatar / Profile Indicator */}
          <button
            type="button"
            ...
            title="Меню и профиль кипера (Экран 7)"
            aria-label="Меню и профиль"
          >
            SL
          </button>

          {/* 2. Thin Screen Progress Strip (7 reels slides) */}
      ...
      {/* SCREEN 3: KITCHEN & RATION (ALL-IN-ONE SCREEN) */}
      <KitchenSlide
